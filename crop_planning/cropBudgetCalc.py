...
class CropBudgetCalc(object):
    ...
    def __init__(self,**kwargs):
        """
        Acceptable Kwargs
        seed_check, seed_cost, seedling_check, num_plants_acre, per_plant_cost, sell_price_min,\ 
    ...
    def profitLossData(self):
        ...
        prod_min, prod_max = self.calcProduction()
        data = [
            ["Normal Profit" , round(norm_profit)] ,
            ["Monthly Normal Profit" , round(norm_profit/self.crop_duration)] ,
            ["MAX Profit" , round(max_profit) ],
            ["MIN Profit" , round(min_profit) ],
            ["NORMAL Revenue" , round(rev_norm)] ,
            ["Max Production Per Acre",  round(prod_max)] ,
            ["Min Production Per Acre",  round(prod_min)] ,
            ["Cost", round(self.cropInputCost())] ,
            ["Min Production Cost per kg",round(self.cropInputCost()/prod_max)],
            ["Max Production Cost per kg",round(self.cropInputCost()/prod_min)],
